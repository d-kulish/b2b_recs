-- Probability-to-buy test view v2: positives (label=1) + sampled negatives (label=0)
-- Purpose: Held-out evaluation set for ranking model predicting purchase probability
--
-- Data source: tfrs_training_examples_v2_Ternopil (only the last day)
...
-- Negative sampling: Deterministic via FARM_FINGERPRINT with 'test' salt (different negatives from train)
--
-- v2 change: Product catalog restricted to top 80% products by cumulative revenue.
--   This ensures the 1:4 ratio is preserved when the dataset config applies the same
--   top-80% revenue filter, avoiding the ratio distortion seen in v1.

CREATE OR REPLACE VIEW `b2b-recs.raw_data.ternopil_prob_test_v2` AS

WITH test_data AS (
  -- Only the last day (same filter as ternopil_test)
  SELECT *
...
-- Ranked deterministically per customer via FARM_FINGERPRINT with 'test' salt
...
negative_ranked AS (
  SELECT
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY FARM_FINGERPRINT(CONCAT('test_', CAST(c.customer_id AS STRING), '_', CAST(p.product_id AS STRING)))
    ) AS neg_rank
