-- Probability-to-buy test view v4: positives (label=1) + sampled negatives (label=0)
-- Purpose: Held-out evaluation set for ranking model predicting purchase probability
--
-- Data source: tfrs_training_examples_v3_Ternopil (only the last day)
...
WITH test_data AS (
  -- Only the last day (same filter as ternopil_test_v3)
  SELECT *
...
-- From HISTORICAL data (training period), not test day — point-in-time correctness
...
-- purchase_history is the buyer's full history from training period
negatives AS (
  SELECT
