-- Training view v5: all data EXCEPT the last day
-- Purpose: Training set that excludes the most recent day (used for evaluation)
-- v5 changes vs v4:
--   - purchase_history now INCLUDES the target product (per-customer, not per-row)
--   - Rationale: at serving time, the target is unknown and cannot be excluded.
--     Excluding it created a "missing product" signal that broke serving accuracy.
--     Including it is safe because mean pooling over 50 items dilutes any single
--     item to ~2% of the 32D vector — the model cannot exploit presence.
--   - See docs/potential/purchase_history_bug.md Bug #2 for full analysis.
--
-- All other features unchanged from v4:
--   - Product aggregate features: prod_unique_buyers, prod_order_count, prod_total_sales,
...
CREATE OR REPLACE VIEW `b2b-recs.raw_data.ternopil_train_v5` AS
...
-- Purchase history per customer: top 50 products by recency, INCLUDING target
-- This matches serving-time behavior where the full history is used for all candidates.
