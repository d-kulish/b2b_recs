...
- [x] Create Feature Config with clip (lower 1st + upper 90th) + log (no normalize) ✅ Verified 2026-01-07
- [ ] Verify clip UI shows lower/upper percentile selectors
- [x] Verify transform code uses `tft.quantiles()` (not `tft.max() * 0.99`) ✅ Verified 2026-01-07
...
| Cross: `customer_id × cust_value` | cross | Hash(5000) + Embed | 16D |
...
| Cross: `category × sub_category` | cross | Hash(1000) + Embed | 16D |

**Verification Checks:**

| Check | Result | Notes |
|-------|--------|-------|
| `config_type` derived correctly | ✅ Pass | Auto-set to `ranking` from target_column presence |
| Target column NOT in buyer_model_features | ✅ Pass | `sales` not in buyer columns |
| Target column NOT in product_model_features | ✅ Pass | `sales` not in product columns |
| Transform code includes TARGET COLUMN section | ✅ Pass | After regeneration |
| Transform code uses `tft.quantiles()` | ✅ Pass | With index 9 (1st%) and 899 (90th%) |
| Transform code uses `tf.math.log1p()` | ✅ Pass | For log transform |
| Trainer code includes `LABEL_KEY` | ✅ Pass | `LABEL_KEY = 'sales_target'` |
| Trainer code includes `RankingModel` | ✅ Pass | With `tfrs.tasks.Ranking` |
