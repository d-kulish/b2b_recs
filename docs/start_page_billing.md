...
|                                                                  |
|  +- Summary Bar -----------------------------------------------+|
|  |  Period: Feb 1-28  |  Estimated: EUR178  |  License: -100%   ||
...
**Location:** `b2b-recs.billing_export` — a dedicated dataset in the `b2b-recs` development project.

**Setup steps:**
1. Go to GCP Console > Billing > Billing Export > BigQuery Export
2. Create dataset `billing_export` in project `b2b-recs`
3. Enable "Standard usage cost" export

**Data availability:** ~24 hours delay. The daily collection job (02:00 UTC) processes the previous day's costs.
...
**BillingConfig** — Singleton configuration for the billing model.
...
**BillingSnapshot** — Daily materialized cost records per GCP service.
...
| `gcp_cost` | DecimalField | Actual GCP charge for this service on this date |
...
The billing snapshot collection extends the existing daily job pattern (02:00 UTC via Cloud Scheduler).

### Flow

1. Query `b2b-recs.billing_export.gcp_billing_export_v1_XXXXXX` for yesterday's costs, filtered by `project.id`
2. For each GCP service with non-zero cost:
   - Look up margin: 50% for `Vertex AI`, 100% for everything else (from `BillingConfig`)
   - Calculate `platform_fee = gcp_cost * margin_pct / 100`
   - Create or update `BillingSnapshot` record
3. Return status (success / partial / error), same pattern as `collect_resource_metrics`

### Integration Options

**Option A:** Extend `collect_resource_metrics` management command to also collect billing snapshots after resource metrics.

**Option B:** Separate `collect_billing_snapshots` management command with its own scheduler job.

Option A is simpler (reuses the scheduler). Option B provides better isolation (billing failures don't affect resource metrics). Decide at implementation time.
...
### Phase 1: GCP Setup
- [ ] Enable GCP Billing Export to BigQuery in billing account settings
- [ ] Create `billing_export` dataset in `b2b-recs` project
...
### Phase 2: Data Layer
- [ ] Create `BillingConfig` model (singleton)
- [ ] Create `BillingSnapshot` model
- [ ] Run migrations
- [ ] Create management command `collect_billing_snapshots`
- [ ] Backfill command for historical billing data (as far back as export has data)
...
- [ ] Add billing snapshot collection to daily 02:00 UTC job (or create separate scheduler)
...
- Currency: **EUR** throughout (€ symbol)
