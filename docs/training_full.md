...
│  │                      DJANGO BACKEND                                  │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐               │   │
│  │  │ Training API │  │  Training    │  │   Code       │               │   │
│  │  │  (api.py)    │  │  Service     │  │  Generators  │               │   │
│  │  └──────────────┘  └──────────────┘  └──────────────┘               │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                              │                                               │
│                              ▼                                               │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                    CLOUD BUILD                                       │   │
│  │  Pre-built GPU image: tfx-trainer-gpu:latest                        │   │
│  │  Compile TFX pipeline → Submit to Vertex AI                         │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                              │                                               │
│                              ▼                                               │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                 VERTEX AI PIPELINES (7 stages)                       │   │
│  │                                                                      │   │
│  │  ExampleGen → Stats → Schema → Transform → Trainer → Evaluator      │   │
│  │                                                          │           │   │
│  │                                                          ▼           │   │
│  │                                                       Pusher         │   │
│  │                                                          │           │   │
...
│  USER INPUT (Wizard)                                                         │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ • Training run name (→ model name, endpoint ID)                      │   │
│  │ • Base experiment (→ Dataset, FeatureConfig, ModelConfig)           │   │
│  │ • Training params (epochs=150, batch_size, LR, early_stopping)      │   │
│  │ • Advanced params (LR schedule, mixed precision, warm restart)       │   │
│  │ • GPU config (type, count, preemptible)                              │   │
│  │ • Deployment options (register only / deploy)                        │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                              │                                               │
│                              ▼                                               │
│  CODE GENERATION                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ 1. PreprocessingFnGenerator → transform_module.py                    │   │
│  │    Source: ml_platform/configs/services.py:840                       │   │
│  │    - Vocabulary computation, normalization, bucketization            │   │
│  │    - Cross features, cyclical encoding                               │   │
│  │                                                                      │   │
│  │ 2. TrainerModuleGenerator → trainer_module.py (EXTENDED)             │   │
│  │    Source: ml_platform/configs/services.py:1529                      │   │
│  │    Extensions:                                                       │   │
│  │    - Early stopping callback                                         │   │
│  │    - LR scheduling (ReduceOnPlateau, Cosine, Warmup)                │   │
│  │    - Checkpointing for preemptible recovery                         │   │
│  │    - Mixed precision support                                         │   │
│  │    - Multi-GPU MirroredStrategy                                     │   │
│  │    - Warm restart weight loading                                     │   │
│  │                                                                      │   │
│  │ 3. BigQueryService → SQL query                                       │   │
│  │    Source: ml_platform/datasets/services.py:1435                     │   │
│  │    - 100% data (no sampling)                                         │   │
│  │    - Strict time split for production                                │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                              │                                               │
│                              ▼                                               │
│  GCS UPLOAD                                                                  │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ gs://b2b-recs-training-artifacts/{run_id}/                           │   │
│  │ ├── transform_module.py                                              │   │
│  │ ├── trainer_module.py                                                │   │
...
│                              │                                               │
│                              ▼                                               │
│  CLOUD BUILD (compile_and_submit.py)                                        │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Image: europe-central2-docker.pkg.dev/b2b-recs-platform/             │   │
│  │        tfx-builder/tfx-trainer-gpu:latest                            │   │
│  │                                                                      │   │
│  │ Steps:                                                               │   │
│  │ 1. Create 7-stage TFX pipeline (with Evaluator + Pusher)            │   │
│  │ 2. Configure GPU for Trainer component                               │   │
│  │ 3. Compile to Kubeflow v2 IR (JSON)                                 │   │
│  │ 4. Submit to Vertex AI Pipelines                                    │   │
│  │ 5. Write result to GCS                                               │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                              │                                               │
│                              ▼                                               │
│  VERTEX AI PIPELINE EXECUTION                                                │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Stage 1: BigQueryExampleGen                                          │   │
│  │          BigQuery → TFRecords (train/eval/test splits)              │   │
│  │          Infrastructure: Dataflow (auto-scaling)                     │   │
│  │                                                                      │   │
│  │ Stage 2: StatisticsGen                                               │   │
│  │          TFRecords → TFDV statistics                                │   │
...
│  │                                                                      │   │
│  │ Stage 4: Transform                                                   │   │
│  │          Apply preprocessing_fn, build vocabularies                 │   │
│  │          Infrastructure: Dataflow                                    │   │
│  │                                                                      │   │
│  │ Stage 5: Trainer (GPU)                                               │   │
│  │          Train TFRS model with GPU acceleration                     │   │
│  │          Infrastructure: Vertex AI Custom Job (GPU VM)              │   │
│  │          - Checkpointing to GCS every 10 epochs                     │   │
│  │          - Early stopping monitoring                                 │   │
│  │          - Webhook callbacks for progress updates                   │   │
│  │                                                                      │   │
│  │ Stage 6: Evaluator                                                   │   │
│  │          TFMA evaluation, model blessing check                       │   │
│  │          - Compare against blessing threshold (e.g., R@100 > 0.40)  │   │
│  │                                                                      │   │
│  │ Stage 7: Pusher (conditional)                                        │   │
│  │          If blessed: Push to Vertex Model Registry                  │   │
│  │          If not blessed: Skip, mark training with warning           │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                              │                                               │
│                              ▼                                               │
│  OUTPUT ARTIFACTS                                                            │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ GCS: gs://b2b-recs-training-artifacts/{run_id}/                      │   │
│  │ ├── model/saved_model/                                               │   │
│  │ ├── model/query_model/                                               │   │
...
│  │                                                                      │   │
│  │ Vertex Model Registry:                                               │   │
│  │ └── models/{training_run_name}/versions/{version}                   │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  Browser (model_training.html)                                              │
│       │                                                                      │
│       │ POST /api/training-runs/                                            │
│       │ {name, base_experiment_id, training_params, gpu_config, ...}       │
│       │                                                                      │
│       ▼                                                                      │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
...
│  │     │                                                                │   │
│  │     ├─► Validate inputs                                              │   │
│  │     ├─► Create TrainingRun record (status=PENDING)                  │   │
│  │     └─► Call TrainingService.submit()                               │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│       │                                                                      │
│       ▼                                                                      │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ ml_platform/training/services.py                                     │   │
│  │   TrainingService.submit()                                           │   │
│  │     │                                                                │   │
│  │     ├─► Load FeatureConfig, ModelConfig, Dataset                    │   │
│  │     │                                                                │   │
│  │     ├─► PreprocessingFnGenerator.generate()                         │   │
│  │     │   Source: ml_platform/configs/services.py                      │   │
│  │     │   Output: transform_module.py                                  │   │
│  │     │                                                                │   │
│  │     ├─► TrainerModuleGenerator.generate() [EXTENDED]                │   │
│  │     │   Source: ml_platform/configs/services.py                      │   │
│  │     │   Extensions: GPU, checkpointing, LR schedule                 │   │
│  │     │   Output: trainer_module.py                                    │   │
│  │     │                                                                │   │
│  │     ├─► BigQueryService.generate_training_query()                   │   │
│  │     │   Source: ml_platform/datasets/services.py                     │   │
│  │     │   Output: SQL query (100% data, strict time split)            │   │
│  │     │                                                                │   │
│  │     ├─► Upload to GCS                                               │   │
│  │     │   gs://b2b-recs-training-artifacts/{run_id}/                  │   │
│  │     │                                                                │   │
│  │     └─► Trigger Cloud Build                                         │   │
│  │         cloudbuild_v1.CloudBuildClient().create_build()             │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│       │                                                                      │
│       ▼                                                                      │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Cloud Build (compile_and_submit.py)                                  │   │
│  │   Image: tfx-trainer-gpu:latest                                      │   │
│  │     │                                                                │   │
│  │     ├─► Create TFX Pipeline (7 components)                          │   │
│  │     ├─► Configure GPU for Trainer                                   │   │
│  │     ├─► Compile with KubeflowV2DagRunner                            │   │
│  │     ├─► Submit to Vertex AI: aiplatform.PipelineJob().submit()      │   │
│  │     └─► Write result to GCS                                         │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│       │                                                                      │
│       ▼                                                                      │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Vertex AI Pipelines                                                  │   │
│  │   Pipeline execution (2-8 hours)                                     │   │
│  │     │                                                                │   │
│  │     └─► On completion: artifacts saved, model registered            │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│       │                                                                      │
│       ▼                                                                      │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Status Polling (Browser)                                             │   │
│  │   GET /api/training-runs/{id}/ every 30 seconds                     │   │
│  │     │                                                                │   │
│  │     ├─► Check Cloud Build result (gs://.../{run_id}.json)           │   │
│  │     ├─► Query Vertex AI pipeline status                             │   │
│  │     ├─► Extract task progress (per-component)                       │   │
│  │     └─► On completion: extract results, update TrainingRun          │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ [ Best Experiments ] [ Training ] [ Models Registry ]                │   │
│  │        (Ch. 1)         (Ch. 2)        (Ch. 3)                        │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                                                                      │   │
│  │  CHAPTER CONTENT (switched by tab selection)                         │   │
│  │                                                                      │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  KPI Rows (Retrieval / Ranking / Hybrid)                                    │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ 🔍 RETRIEVAL │ Exps: 15 │ R@5: 0.234 │ R@10: 0.412 │ R@100: 0.823  │   │
│  ├─────────────────────────────────────────────────────────────────────┤   │
│  │ 📊 RANKING   │ Exps: 8  │ RMSE: 0.452 │ Test RMSE: 0.478 │ ...     │   │
│  ├─────────────────────────────────────────────────────────────────────┤   │
│  │ 📚 HYBRID    │ Exps: 5  │ R@50: 0.654 │ R@100: 0.801 │ RMSE: 0.423 │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│  Top Configurations Table (10 rows, scrollable)                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ # │ Experiment │ Dataset │ Feature │ Model │ LR │ Batch │ R@100    │   │
│  │ 1 │ Exp #47    │ Q4 Data │ cfg-042 │ mdl-1 │0.1 │ 8192  │ 0.823    │   │
│  │ 2 │ Exp #45    │ Q4 Data │ cfg-038 │ mdl-2 │0.05│ 4096  │ 0.801    │   │
│  │ ...                                                                  │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│  Click row → Opens ExpViewModal (reusable module)                           │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Filter Bar                                                           │   │
│  │ [Status: All ▼] [Model Type: All ▼] [Dataset: All ▼]  [🔍 Search]   │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│                                                   [+ New Training Run]      │
│                                                                              │
│  Training Run Cards                                                          │
│  ┌────────────────────────────────────────────────────────────────────────┐│
│  │ 🔄 product-recommender-v3                                     Running  ││
│  │ Based on: Exp #47 │ Dataset: Q4 2024 │ GPU: 2x L4                      ││
│  │ ████████████████░░░░░░░░░░░░░░░░ 55% (Epoch 82/150)                    ││
│  │ Started: 4h 12m ago │ Current R@100: 0.412                             ││
│  │ [View]                                                       [Cancel]  ││
│  └────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐│
│  │ ✅ product-recommender-v2                          Completed • Deployed ││
│  │ Based on: Exp #45 │ Dataset: Q4 2024 │ GPU: 2x T4                      ││
│  │ Duration: 6h 28m │ Final R@100: 0.468 │ vs Exp: +0.021                 ││
│  │ Model: models/product-recommender-v2/1                                 ││
│  │ [View] [Deploy to Endpoint]                                            ││
│  └────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐│
│  │ ⏸️ cross-sell-ranker-v1                                      Scheduled ││
│  │ Based on: Exp #62 │ Dataset: Q4 2024 │ GPU: 4x L4                      ││
│  │ Scheduled: Tomorrow 02:00 UTC (off-peak)                               ││
│  │ [View] [Run Now]                                             [Cancel]  ││
│  └────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐│
│  │ ❌ ranking-model-v1                                             Failed  ││
│  │ Based on: Exp #58 │ Dataset: Q3 2024 │ GPU: 2x T4                      ││
│  │ Failed at: Trainer (epoch 45) │ Error: CUDA OOM                        ││
│  │ [View Logs] [Retry with more GPU memory]                               ││
│  └────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
│  Empty State (when no training runs):                                        │
│  ┌────────────────────────────────────────────────────────────────────────┐│
│  │                        🚀                                              ││
│  │           No training runs yet                                         ││
│  │   Run experiments first, then promote the best to full training       ││
│  │                  [+ New Training Run]                                  ││
│  └────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
...
│                                                                              │
│ ┌─────────────────────────────────────────────────────────────────────────┐│
│ │ product-recommender-v3                                                   ││
│ └─────────────────────────────────────────────────────────────────────────┘│
│ ℹ️ This name will be used as model name and endpoint identifier            │
│   Allowed: lowercase letters, numbers, hyphens (e.g., my-model-v2)         │
│                                                                              │
│ ═══════════════════════════════════════════════════════════════════════════ │
│ MODEL TYPE *                                                                 │
│ ═══════════════════════════════════════════════════════════════════════════ │
...
│                                                                              │
│ Search: ┌─────────────────────────────────────────────────────────────────┐│
│         │ 🔍 Search by name, number, config... (e.g., "Exp #65")          ││
│         └─────────────────────────────────────────────────────────────────┘│
│                                                                              │
│ Top 5 Retrieval Experiments:                                                │
│ ┌─────────────────────────────────────────────────────────────────────────┐│
│ │ ● │ Exp #47 │ Q4 Data │ cfg-042 │ R@100: 0.823 │ 20 ep │ [👁 View]     ││
│ ├─────────────────────────────────────────────────────────────────────────┤│
│ │ ○ │ Exp #45 │ Q4 Data │ cfg-038 │ R@100: 0.801 │ 15 ep │ [👁 View]     ││
│ ├─────────────────────────────────────────────────────────────────────────┤│
│ │ ○ │ Exp #42 │ Q3 Data │ cfg-035 │ R@100: 0.798 │ 25 ep │ [👁 View]     ││
│ ├─────────────────────────────────────────────────────────────────────────┤│
│ │ ○ │ Exp #38 │ Q4 Data │ cfg-032 │ R@100: 0.785 │ 10 ep │ [👁 View]     ││
│ ├─────────────────────────────────────────────────────────────────────────┤│
│ │ ○ │ Exp #35 │ Q3 Data │ cfg-028 │ R@100: 0.772 │ 20 ep │ [👁 View]     ││
│ └─────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
│ Selected Experiment Summary:                                                 │
│ ┌─────────────────────────────────────────────────────────────────────────┐│
│ │ Exp #47 - Best Retrieval Model                                          ││
│ │ • Dataset: Q4 2024 Training Data (2.45M rows, 98K users, 36K products) ││
│ │ • Features: config-042 (Large embeddings, user_id: 64d, product_id: 64d)││
│ │ • Model: mdl-retrieval-001 (128→64→32 towers, Adagrad optimizer)       ││
│ │ • Results: R@100: 0.823, R@50: 0.687, Loss: 0.31                       ││
│ └─────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
│                                                                              │
│                                                    [Cancel]  [Next →]       │
...
- **[👁 View] button**: Opens `ExpViewModal` with full experiment details
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│ New Training Run                                              Step 2 of 3   │
│ CONFIGURATION & PARAMETERS                                    ───●───●───○  │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│ ═══════════════════════════════════════════════════════════════════════════ │
│ INHERITED CONFIGURATION (from Exp #47)                                       │
│ ═══════════════════════════════════════════════════════════════════════════ │
│                                                                              │
│ Dataset                                                           [Change]  │
│ ┌─────────────────────────────────────────────────────────────────────────┐│
│ │ 📊 Q4 2024 Training Data                                                ││
│ │    2.45M rows │ 98K users │ 36K products │ Last 6 months               ││
│ └─────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
│ Feature Config                                                    [Change]  │
│ ┌─────────────────────────────────────────────────────────────────────────┐│
│ │ ⚙️ config-042: Large embeddings                                         ││
│ │    user_id: 64d │ product_id: 64d │ crosses: cat×subcat, user×city     ││
│ │    Buyer tensor: 192d │ Product tensor: 160d                            ││
│ └─────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
│ Model Config                                                      [Change]  │
│ ┌─────────────────────────────────────────────────────────────────────────┐│
│ │ 🏗️ mdl-retrieval-001: Standard towers                                   ││
│ │    Buyer: 128→64→32 │ Product: 128→64→32 │ Optimizer: Adagrad          ││
│ │    Total params: ~2.4M                                                  ││
│ └─────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
│ ═══════════════════════════════════════════════════════════════════════════ │
...
│ │ Data Sample                 │  │ Epochs                      │            │
│ │ [100% ▼]                    │  │ [150 ▼]                     │            │
│ │ (Exp used: 10%)             │  │ (Exp used: 20) ⚠️ INCREASED │            │
│ └─────────────────────────────┘  └─────────────────────────────┘            │
│                                                                              │
│ ┌─────────────────────────────┐  ┌─────────────────────────────┐            │
│ │ Batch Size                  │  │ Learning Rate               │            │
│ │ [8192 ▼]                    │  │ [0.1 ▼]                     │            │
│ │ (Exp used: 8192)            │  │ (Exp used: 0.1)             │            │
│ └─────────────────────────────┘  └─────────────────────────────┘            │
│                                                                              │
│ Split Strategy: [Strict Time ▼]                                             │
│   └─ Train: 80% │ Validation: 15% │ Test: 5%                               │
│                                                                              │
│ ☑ Early Stopping                                                            │
│   └─ Patience: [10 ▼] epochs (stop if no improvement for N epochs)         │
│                                                                              │
│ ═══════════════════════════════════════════════════════════════════════════ │
...
│ ● Reduce on Plateau                                                         │
│   └─ Factor: 0.5 │ Patience: 3 epochs │ Min LR: 0.001                      │
│ ○ Cosine Decay                                                              │
│   └─ Decay to 1% of initial LR over training                               │
│ ○ Warmup + Cosine                                                           │
│   └─ Linear warmup for 1000 steps, then cosine decay                       │
│                                                                              │
│ ☐ Mixed Precision Training (FP16)                                           │
│   └─ ~2x faster on supported GPUs (T4, L4, V100, A100)                     │
│                                                                              │
│ ☐ Warm Restart from Experiment                                              │
│   └─ Load pretrained weights from Exp #47                                   │
│   └─ ⚠️ Falls back to fresh start if artifacts unavailable                 │
│                                                                              │
│ Retrieval Index (for Retrieval/Hybrid models):                              │
│ ○ Brute Force (exact search, best for <10K products)                        │
│ ● ScaNN (approximate search, 10K+ products)                                 │
│   └─ Num leaves: [Auto ▼]  Num leaves to search: [Auto ▼]                  │
│                                                                              │
│                                                                              │
│                                            [← Back]  [Cancel]  [Next →]     │
│                                                                              │
...
│ ┌─────────────────────────────────────────────────────────────────────────┐│
│ │ ○ T4 (16GB each)                                                        ││
│ │   Good availability, solid performance                                  ││
│ ├─────────────────────────────────────────────────────────────────────────┤│
│ │ ● L4 (24GB each)                                         ⭐ Recommended ││
│ │   Best value, great for large embeddings                                ││
│ ├─────────────────────────────────────────────────────────────────────────┤│
│ │ ○ V100 (16GB each)                                                      ││
│ │   Highest performance, limited availability                             ││
│ ├─────────────────────────────────────────────────────────────────────────┤│
│ │ ○ A100 (40GB each)                                                      ││
│ │   Very large models only, premium pricing                               ││
│ └─────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
│ Number of GPUs:  [2 ▼]   Options: 1, 2, 4, 8                               │
│                          Default: 2 (single VM)                             │
│                                                                              │
│ ☑ Use Preemptible/Spot VMs                                                 │
│   └─ 70% cost reduction, may be interrupted                                │
│   └─ ℹ️ Checkpointing enabled to recover from interruptions                │
│                                                                              │
│ ═══════════════════════════════════════════════════════════════════════════ │
│ EVALUATOR                                                                    │
│ ═══════════════════════════════════════════════════════════════════════════ │
│                                                                              │
│ ☑ Enable Model Evaluation (TFMA)                                            │
│   └─ Run TensorFlow Model Analysis after training                          │
│                                                                              │
│ Model Blessing Threshold:                                                    │
│   For Retrieval: R@100 must exceed [0.40 ▼] to be pushed to registry       │
│   ℹ️ Models below threshold will be saved but marked as "not blessed"      │
│                                                                              │
│ ═══════════════════════════════════════════════════════════════════════════ │
│ DEPLOYMENT                                                                   │
│ ═══════════════════════════════════════════════════════════════════════════ │
...
│ ● Register model only (deploy manually later)                               │
│ ○ Deploy to new endpoint: product-recommender-v3                            │
│ ○ Deploy to existing endpoint: [production-recs ▼]                          │
│   └─ Traffic split: [10% ▼] (gradual rollout)                              │
│                                                                              │
│ ═══════════════════════════════════════════════════════════════════════════ │
│ SCHEDULE                                                                     │
│ ═══════════════════════════════════════════════════════════════════════════ │
...
│ ● Run immediately                                                            │
│ ○ Schedule for later:                                                        │
│   Date: [2026-01-17 ▼]  Time: [02:00 ▼] UTC                                │
│   └─ ℹ️ Off-peak hours may have better GPU availability                    │
│                                                                              │
│ ═══════════════════════════════════════════════════════════════════════════ │
│ SUMMARY                                                                      │
│ ═══════════════════════════════════════════════════════════════════════════ │
│                                                                              │
│ ┌─────────────────────────────────────────────────────────────────────────┐│
│ │ Training Run: product-recommender-v3                                    ││
│ │ Based on: Exp #47 (R@100: 0.823)                                        ││
│ │                                                                          ││
│ │ Configuration:                                                           ││
│ │ • Dataset: Q4 2024 Training Data (100%, 2.45M rows)                     ││
│ │ • Features: config-042 (Large embeddings)                               ││
│ │ • Model: mdl-retrieval-001 (Standard towers)                            ││
│ │                                                                          ││
│ │ Training:                                                                ││
│ │ • 150 epochs, batch 8192, LR 0.1 (ReduceOnPlateau)                      ││
│ │ • Early stopping: patience 10                                           ││
│ │ • Index: ScaNN                                                          ││
│ │                                                                          ││
│ │ Infrastructure:                                                          ││
│ │ • GPU: 2x L4 (preemptible)                                              ││
│ │ • Evaluator: Enabled (threshold R@100 > 0.40)                           ││
│ │                                                                          ││
│ │ Post-training:                                                           ││
│ │ • Register model to Vertex Registry                                     ││
│ │ • Deploy: Manual                                                        ││
│ └─────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
│                                                                              │
│                                     [← Back]  [Cancel]  [▶ Run Training]    │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│ Select Feature Config                                                   [X] │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│ Search: [🔍 Search configs...                                            ]  │
│                                                                              │
│ ┌─────────────────────────────────────────────────────────────────────────┐│
│ │ ● config-042: Large embeddings                               ⭐ Current ││
│ │   user_id: 64d │ product_id: 64d │ Buyer: 192d │ Product: 160d         ││
│ ├─────────────────────────────────────────────────────────────────────────┤│
│ │ ○ config-038: Standard embeddings                                       ││
│ │   user_id: 32d │ product_id: 32d │ Buyer: 128d │ Product: 96d          ││
│ ├─────────────────────────────────────────────────────────────────────────┤│
│ │ ○ config-035: Minimal embeddings                                        ││
│ │   user_id: 16d │ product_id: 16d │ Buyer: 64d │ Product: 48d           ││
│ └─────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
│                                                      [Cancel]  [Select]     │
│                                                                              │
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         TRAINING RUN CARD STATES                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│ STATUS: PENDING                                                              │
│ ┌────────────────────────────────────────────────────────────────────────┐ │
│ │ ⏳ product-recommender-v3                                      Pending  │ │
│ │ Based on: Exp #47 │ Dataset: Q4 2024 │ GPU: 2x L4                      │ │
...
│ STATUS: SCHEDULED                                                            │
│ ┌────────────────────────────────────────────────────────────────────────┐ │
│ │ ⏰ cross-sell-ranker-v1                                      Scheduled  │ │
│ │ Based on: Exp #62 │ Dataset: Q4 2024 │ GPU: 4x L4                      │ │
│ │ Scheduled: Jan 17, 2026 02:00 UTC (in 14 hours)                        │ │
│ │ [View] [Run Now]                                             [Cancel]  │ │
│ └────────────────────────────────────────────────────────────────────────┘ │
│                                                                              │
│ STATUS: RUNNING                                                              │
...
│ │ Based on: Exp #47 │ Dataset: Q4 2024 │ GPU: 2x L4                      │ │
│ │ ████████████████░░░░░░░░░░░░░░░░ 55% (Epoch 82/150)                   │ │
│ │ Stage: Trainer │ Started: 4h 12m ago │ Current R@100: 0.412           │ │
│ │ [View]                                                       [Cancel]  │ │
│ └────────────────────────────────────────────────────────────────────────┘ │
│                                                                              │
│ STATUS: COMPLETED                                                            │
│ ┌────────────────────────────────────────────────────────────────────────┐ │
│ │ ✅ product-recommender-v2                                    Completed  │ │
│ │ Based on: Exp #45 │ Dataset: Q4 2024 │ GPU: 2x T4                      │ │
│ │ Duration: 6h 28m │ Final R@100: 0.468 │ vs Exp: +0.021 ↑              │ │
│ │ Model: models/product-recommender-v2/1                                 │ │
│ │ [View] [Deploy to Endpoint]                                            │ │
...
│ ┌────────────────────────────────────────────────────────────────────────┐ │
│ │ ✅ product-recommender-v1                          Completed • Deployed │ │
│ │ Based on: Exp #38 │ Dataset: Q3 2024 │ GPU: 2x T4                      │ │
│ │ Duration: 5h 45m │ Final R@100: 0.445 │ Endpoint: production-recs     │ │
│ │ [View] [Manage Endpoint]                                               │ │
│ └────────────────────────────────────────────────────────────────────────┘ │
│                                                                              │
│ STATUS: FAILED                                                               │
│ ┌────────────────────────────────────────────────────────────────────────┐ │
│ │ ❌ ranking-model-v1                                              Failed  │ │
│ │ Based on: Exp #58 │ Dataset: Q3 2024 │ GPU: 2x T4                      │ │
│ │ Failed at: Trainer (epoch 45) │ Error: CUDA out of memory             │ │
│ │ [View Logs] [Retry with more GPU memory]                               │ │
│ └────────────────────────────────────────────────────────────────────────┘ │
│                                                                              │
│ STATUS: CANCELLED                                                            │
│ ┌────────────────────────────────────────────────────────────────────────┐ │
│ │ ⊘ test-model-v1                                              Cancelled  │ │
│ │ Based on: Exp #55 │ Dataset: Q4 2024 │ GPU: 1x T4                      │ │
│ │ Cancelled at: Trainer (epoch 12) │ By: user@example.com               │ │
│ │ [View]                                                                 │ │
│ └────────────────────────────────────────────────────────────────────────┘ │
│                                                                              │
│ STATUS: NOT BLESSED                                                          │
│ ┌────────────────────────────────────────────────────────────────────────┐ │
│ │ ⚠️ low-perf-model-v1                                       Not Blessed  │ │
│ │ Based on: Exp #52 │ Dataset: Q3 2024 │ GPU: 2x T4                      │ │
│ │ Duration: 4h 15m │ Final R@100: 0.285 │ Threshold: 0.40               │ │
│ │ ⚠️ Model saved but not pushed to registry (below threshold)            │ │
│ │ [View] [Push Anyway]                                                   │ │
│ └────────────────────────────────────────────────────────────────────────┘ │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
...
│   └── _pipeline_dag.html     # (existing, reuse)
│
static/
├── js/
...
│
docs/
└── training_full.md           # This document
```
