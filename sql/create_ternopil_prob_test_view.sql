-- Probability-to-buy test view: positives (label=1) + sampled negatives (label=0)
...
-- Product catalog: All products from the full Ternopil table (not just the test day)

CREATE OR REPLACE VIEW `b2b-recs.raw_data.ternopil_prob_test` AS
...
-- Product catalog with metadata (from the full Ternopil table, not just the test day)
product_catalog AS (
