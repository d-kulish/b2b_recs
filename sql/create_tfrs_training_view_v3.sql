-- Create view for TFRS training examples (v3)
...
--
-- v3 changes vs v2:
--   - Excluded division_desc IN ('OTHERS FD', 'OTHERS NF') from products
--     to remove catch-all categories that shouldn't be in recommendations

CREATE OR REPLACE VIEW `b2b-recs.raw_data.tfrs_training_examples_v3` AS
...
products_with_id AS (
  ...
  -- Excludes OTHERS FD and OTHERS NF divisions
  SELECT
    product_id,
    art_name,
    division_desc,
  ...
  FROM (
    ...
    WHERE division_desc NOT IN ('OTHERS FD', 'OTHERS NF')
  )
  WHERE rn = 1
),
...
-- INNER JOIN to exclude invoices for products in OTHERS FD / OTHERS NF
INNER JOIN products_with_id p
